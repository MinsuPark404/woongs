...
from packages.tracker_engine import BYTETracker
...
def demo(video_path, output_path='./out'):
    ...
    tracker = BYTETracker(frame_rate=30.)  # fps 설정
    ...
    for idx in tqdm(range(tot_cap_frame)):
        ...
        if ret:
            ...
            for idx, (box, kpt) in enumerate(zip(bboxes, kpts)):
                ...
                plot_one_box(box[:4].astype(int), frame, color= (0, 0, 255), label = f'{box[4]:.4f}')
            
            track_output = tracker.update(bboxes)
            for idx, track in enumerate(track_output):
                plot_one_box(track.tlbr().astype(int), frame, color= (255, 0, 0), label = f'{track.track_id}')
                # tracker는 파란색으로 표시

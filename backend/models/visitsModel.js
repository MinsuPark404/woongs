...
const getMonthlyVisits = async (month, year) => {
  const [results] = await db.query(visitsQueries.monthlyVisitsQuery, [
    month,
    year,
  ]);
  return results;
...
const getMonthlyDomainVisits = async (month, year) => {
  const [results] = await db.query(visitsQueries.monthlyDomainVisitsQuery, [
    month,
    year,
  ]);
  return results;
...
const getDailyDomainVisits = async (date) => {
  const [results] = await db.query(visitsQueries.dailyDomainVisitsQuery, [
    date,
  ]);
  return results;
};
